...
LANGUAGES = {
    "English": {
        ...
        "ollama_setup_steps": [
            ...
            "Start Ollama by opening the Ollama desktop app, or run `ollama serve` if you use the CLI only. Do not start both on the same port.",
            "Check that the server is reachable: `curl http://localhost:11434/api/tags`.",
            "Use `http://localhost:11434` in this app unless you intentionally configured `OLLAMA_HOST`.",
        ...
        "ollama_url_steps": [
            "If you changed Ollama's bind address, check the configured host in a terminal: `echo $OLLAMA_HOST` on macOS/Linux or `$env:OLLAMA_HOST` in Windows PowerShell.",
            "Paste the full base URL here, for example `http://192.168.1.20:11434`. Do not include `/api/generate`; the app adds that path.",
            "Test the URL with `/api/tags`, for example `curl http://localhost:11434/api/tags`.",
            "Pull the model on the same Ollama server you will call from the app.",
        ...
        "ollama_translation_caption": "Ollama generates the personalized affirmation in English first. For Hindi and Telugu, Sarvam translates your reflection to English and translates the English affirmation back when a Sarvam API key is provided.",
    ...
    "हिन्दी": {
        ...
        "ollama_setup_steps": [
            ...
            "Ollama desktop app खोलकर Ollama start कीजिए, या अगर आप CLI-only setup इस्तेमाल कर रहे हैं तो `ollama serve` run कीजिए। एक ही port पर दोनों साथ में start मत कीजिए।",
            "Server reachable है या नहीं check कीजिए: `curl http://localhost:11434/api/tags`.",
            "इस app में `http://localhost:11434` इस्तेमाल कीजिए, जब तक कि आपने जानबूझकर `OLLAMA_HOST` configure ना किया हो।",
        ...
        "ollama_url_steps": [
            "अगर आपने Ollama bind address बदला है, तो terminal में configured host check कीजिए: `echo $OLLAMA_HOST` on macOS/Linux या `$env:OLLAMA_HOST` in Windows PowerShell.",
            "यहाँ full base URL paste कीजिए, जैसे `http://192.168.1.20:11434`. `/api/generate` मत जोड़िए; app वह path खुद add करता है।",
            "ज़रा URL को `/api/tags` के साथ test कीजिए, जैसे कि `curl http://localhost:11434/api/tags`.",
            "Model उसी Ollama server पर pull कीजिए जिसे app call करेगा।",
        ...
        "ollama_translation_caption": "Ollama पहले personalized affirmation English में generate करता है। Hindi और Telugu के लिए, Sarvam आपकी reflection को English में translate करता है और English affirmation को वापस selected language में translate करता है, जब Sarvam API key दी जाती है।",
        "sarvam_key_label": "Sarvam API key",
    ...
    "తెలుగు": {
        ...
        "ollama_setup_steps": [
            ...
            "Ollama desktop app open చేసి Ollama start చేయండి, లేదా CLI-only setup అయితే `ollama serve` run చేయండి. ఒకే port మీద రెండింటినీ కలిసి start చేయకండి.",
            "Server reachable ఉందో లేదో check చేయండి: `curl http://localhost:11434/api/tags`.",
            "మీరు కావాలని `OLLAMA_HOST` configure చేయకపోతే ఈ app-లో `http://localhost:11434` వాడండి.",
        ...
        "ollama_url_steps": [
            "మీరు Ollama bind address మార్చితే, configured host-ని terminal-లో check చేయండి: `echo $OLLAMA_HOST` on macOS/Linux లేదా Windows PowerShell-లో `$env:OLLAMA_HOST`.",
            "ఇక్కడ full base URL paste చేయండి, ఉదాహరణకి `http://192.168.1.20:11434`. `/api/generate` add చేయకండి; app ఆ path-ని add చేస్తుంది.",
            "ఉదాహరణకి `curl http://localhost:11434/api/tags` లాంటి `/api/tags`-తో URL test చేయండి.",
            "App call చేసే అదే Ollama server మీద model pull చేయండి.",
        ...
        "ollama_translation_caption": "Ollama ముందుగా personalized affirmation-ని English-లో generate చేస్తుంది. Hindi, Telugu కోసం Sarvam API key ఇచ్చినప్పుడు, Sarvam మీ reflection-ని English-కి translate చేసి, English affirmation-ని తిరిగి selected language-కి translate చేస్తుంది.",
        "sarvam_key_label": "Sarvam API key",
